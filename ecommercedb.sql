...
CUST_PHONE varchar(50) not null, CUST_CITY varchar (50) not null, CUST_GENDER char );
...
create table product (PRO_ID int primary key, PRO_NAME varchar(50) not null, PRO_DESC varchar(50) not null,
...
SUPP_PRICE int default 0, foreign key (PRO_ID) references product (PRO_ID), 
foreign key (SUPP_ID) references supplier (SUPP_ID)  );

desc supplier_pricing;

create table `order` (ORD_ID int primary key, ORD_AMOUNT int not null, ORD_DATE date , CUST_ID int, PRICING_ID int,
...
INSERT INTO PRODUCT VALUES(3,"ROG LAPTOP","Windows 10 with 15inch, i7 processor, 1TB SSD",4);
INSERT INTO PRODUCT VALUES(4,"OATS","Highly Nutritious from Nestle",3);
INSERT INTO PRODUCT VALUES(5,"HARRY POTTER","Best Collection of all time by J.K Rowling",1);
INSERT INTO PRODUCT VALUES(6,"MILK","1L Toned MIlk",3);
INSERT INTO PRODUCT VALUES(7,"Boat EarPhones","1.5Meter long Dolby Atmos",4);
INSERT INTO PRODUCT VALUES(8,"Jeans","Stretchable Denim Jeans with various",5);
...
select count(T2.CUST_GENDER ) as numberofcustomers, T2.CUST_GENDER from 
(select T1.CUST_ID, T1.CUST_GENDER, T1.ORD_AMOUNT, T1.CUST_NAME 
from (select `order`.*, customer.CUST_GENDER, customer.CUST_NAME from 
`order` inner join customer where `order`.CUST_ID = customer.CUST_ID having `order`.ORD_AMOUNT >= 3000) 
...
`order`.CUST_ID =2 and `order`.PRICING_ID = supplier_pricing.PRICING_ID 
...
select customer.CUST_NAME, customer.CUST_GENDER from customer where customer.CUST_NAME like 'A%' or 
customer.CUST_NAME like '%A';
